...
class ADSBLolClient:
    ...
    async def fetch_rest(self) -> list[FlightData]:
        ...
        url = f"{ADSB_LOL_REST}/lat/{self._latitude}/lon/{self._longitude}/dist/{self._radius_km}"
    ...
    async def _websocket_loop(self) -> None:
        ...
        url = f"{ADSB_LOL_WS}/lat/{self._latitude}/lon/{self._longitude}/dist/{self._radius_km}"
        headers = {"User-Agent": self._user_agent}
        ...
        while True:
            try:
                async with self._session.ws_connect(url, headers=headers, heartbeat=30) as ws:
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            _LOGGER.warning("ADSB.lol WS error: %s", ws.exception())
                            break
                ...
                _LOGGER.warning("ADSB.lol WebSocket error: %s, reconnecting in %ss", err, self._reconnect_delay)
